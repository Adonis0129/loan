...
contract TroveManager is BaseContract, LiquityBase, CheckContract, ITroveManager {
    ...
    *
    * Where L_FURFI(0) and L_FURUSDDebt(0) are snapshots of L_FURFI and L_FURUSDDebt for the active Trove taken at the instant the stake was made
    */
    uint public L_FURFI;
    ...
    // Object containing the FURFI and FURUSD snapshots for a given active trove
    ...
    uint public lastFURFIError_Redistribution;
    uint public lastFURUSDDebtError_Redistribution;
    ...
    **/
    ...
    struct LiquidationTotals {
        ...
        uint totalFURUSDGasCompensation;
    ...
    // --- Events ---
    event Liquidation(uint _liquidatedDebt, uint _liquidatedColl, uint _collGasCompensation, uint _FURUSDGasCompensation);
    ...
    event TroveSnapshotsUpdated(uint _L_FURFI, uint _L_FURUSDDebt);
        ...
        returns (LiquidationValues memory singleLiquidation)
    {
        ...
        singleLiquidation.FURUSDGasCompensation = FURUSD_GAS_COMPENSATION;
        uint collToLiquidate = singleLiquidation.entireTroveColl.sub(singleLiquidation.collGasCompensation);

        (singleLiquidation.debtToOffset,
        singleLiquidation.collToSendToSP,
        singleLiquidation.debtToRedistribute,
        singleLiquidation.collToRedistribute) = _getOffsetAndRedistributionVals(singleLiquidation.entireTroveDebt, collToLiquidate, _FURUSDInStabPool);
        ...
        returns (LiquidationValues memory singleLiquidation)
    {
        ...
        // If 100% < ICR < MCR, offset as much as possible, and redistribute the remainder
            ...
            singleLiquidation.collToSendToSP,
            singleLiquidation.debtToRedistribute,
            singleLiquidation.collToRedistribute) = _getOffsetAndRedistributionVals(singleLiquidation.entireTroveDebt, vars.collToLiquidate, _FURUSDInStabPool);
        ...
        */
        } else if ((_ICR >= MCR) && (_ICR < _TCR) && (singleLiquidation.entireTroveDebt <= _FURUSDInStabPool)) {
            _movePendingTroveRewardsToActivePool(_activePool, _defaultPool, vars.pendingDebtReward, vars.pendingCollReward);
    ...
    */
        ...
        returns (uint debtToOffset, uint collToSendToSP, uint debtToRedistribute, uint collToRedistribute)
    {
        ...
        *
        *  If the trove's debt is larger than the deposited FURUSD in the Stability Pool:
        *
        *  - Offset an amount of the trove's debt equal to the FURUSD in the Stability Pool
    ...
    /*
    *  Get its offset coll/debt and FURFI gas comp, and close the trove.
    */
    ...
    */
    function liquidateTroves(uint _n) external override {
        ...
        if (vars.recoveryModeAtStart) {
            totals = _getTotalsFromLiquidateTrovesSequence_RecoveryMode(contractsCache, vars.price, vars.FURUSDInStabPool, _n);
        } else { // if !vars.recoveryModeAtStart
            totals = _getTotalsFromLiquidateTrovesSequence_NormalMode(contractsCache.activePool, contractsCache.defaultPool, vars.price, vars.FURUSDInStabPool, _n);
        }
        ...
        emit Liquidation(vars.liquidatedDebt, vars.liquidatedColl, totals.totalCollGasCompensation, totals.totalFURUSDGasCompensation);

        // Send gas compensation to caller
        _sendGasCompensation(contractsCache.activePool, msg.sender, totals.totalFURUSDGasCompensation, totals.totalCollGasCompensation);
    }
    ...
    */
    function batchLiquidateTroves(address[] memory _troveArray) public override {
        ...
        if (vars.recoveryModeAtStart) {
            totals = _getTotalFromBatchLiquidate_RecoveryMode(activePoolCached, defaultPoolCached, vars.price, vars.FURUSDInStabPool, _troveArray);
        } else {  //  if !vars.recoveryModeAtStart
            totals = _getTotalsFromBatchLiquidate_NormalMode(activePoolCached, defaultPoolCached, vars.price, vars.FURUSDInStabPool, _troveArray);
        }
        ...
        // Send gas compensation to caller
        _sendGasCompensation(activePoolCached, msg.sender, totals.totalFURUSDGasCompensation, totals.totalCollGasCompensation);
    }
    ...
    */
    ...
    internal pure returns(LiquidationTotals memory newTotals) {
        ...
        newTotals.totalFURUSDGasCompensation = oldTotals.totalFURUSDGasCompensation.add(singleLiquidation.FURUSDGasCompensation);
    ...
    function _sendGasCompensation(IActivePool _activePool, address _liquidator, uint _FURUSD, uint _FURFI) internal {
        if (_FURUSD > 0) {
            furUSDToken.returnFromPool(gasPoolAddress, _liquidator, _FURUSD);
        }

        if (_FURFI > 0) {
            _activePool.sendFURFI(_liquidator, _FURFI);
        }
    }

    // Move a Trove's pending debt and collateral rewards from distributions, from the Default Pool to the Active Pool
    function _movePendingTroveRewardsToActivePool(IActivePool _activePool, IDefaultPool _defaultPool, uint _FURUSD, uint _FURFI) internal {
    ...
    // Redeem as much collateral as possible from _borrower's Trove in exchange for FURUSD up to _maxFURUSDamount
        ...
        internal returns (SingleRedemptionValues memory singleRedemption)
    {
        ...
        singleRedemption.FURUSDLot = LiquityMath._min(_maxFURUSDamount, Troves[_borrower].debt.sub(FURUSD_GAS_COMPENSATION));
        ...
        // Decrease the debt and collateral of the current Trove according to the FURUSD lot and corresponding FURFI to send
        uint newDebt = (Troves[_borrower].debt).sub(singleRedemption.FURUSDLot);
        uint newColl = (Troves[_borrower].coll).sub(singleRedemption.FURFILot);

        if (newDebt == FURUSD_GAS_COMPENSATION) {
            // No debt left in the Trove (except for the liquidation reserve), therefore the trove gets closed
            _removeStake(_borrower);
            _closeTrove(_borrower, Status.closedByRedemption);
            _redeemCloseTrove(_contractsCache, _borrower, FURUSD_GAS_COMPENSATION, newColl);
    ...
    * The redeemer swaps (debt - liquidation reserve) FURUSD for (debt - liquidation reserve) worth of FURFI, so the FURUSD liquidation reserve left corresponds to the remaining debt.
    * In order to close the trove, the FURUSD liquidation reserve is burned, and the corresponding debt is removed from the active pool.
    * The debt recorded on the trove's struct is zero'd elswhere, in _closeTrove.
    * Any surplus FURFI left in the trove, is sent to the Coll surplus pool, and can be later claimed by the borrower.
    */
    function _redeemCloseTrove(ContractsCache memory _contractsCache, address _borrower, uint _FURUSD, uint _FURFI) internal {
        _contractsCache.furUSDToken.burn(gasPoolAddress, _FURUSD);
        // Update Active Pool FURUSD, and send FURFI to account
        _contractsCache.activePool.decreaseFURUSDDebt(_FURUSD);

        // send FURFI from Active Pool to CollSurplus Pool
        _contractsCache.collSurplusPool.accountSurplus(_borrower, _FURFI);
        _contractsCache.activePool.sendFURFI(address(_contractsCache.collSurplusPool), _FURFI);
    }
    ...
    /* Send _FURUSDamount FURUSD to the system and redeem the corresponding amount of collateral from as many Troves as are needed to fill the redemption
    ...
    * redemption will stop after the last completely redeemed Trove and the sender will keep the remaining FURUSD amount, which they can attempt
    * to redeem later.
    */
        ...
        override
    {
        ...
        // Confirm redeemer's balance is less than total FURUSD supply
        assert(contractsCache.furUSDToken.balanceOf(msg.sender) <= totals.totalFURUSDSupplyAtStart);
        ...
        require(totals.totalFURFIDrawn > 0, "TroveManager: Unable to redeem any amount");
    ...
    function getNominalICR(address _borrower) public view override returns (uint) {
        (uint currentFURFI, uint currentFURUSDDebt) = _getCurrentTroveAmounts(_borrower);
    ...
    function _getCurrentTroveAmounts(address _borrower) internal view returns (uint, uint) {
        ...
        uint currentFURFI = Troves[_borrower].coll.add(pendingFURFIReward);
        uint currentFURUSDDebt = Troves[_borrower].debt.add(pendingFURUSDDebtReward);
    ...
    function _applyPendingRewards(IActivePool _activePool, IDefaultPool _defaultPool, address _borrower) internal {
        if (hasPendingRewards(_borrower)) {
            ...
            Troves[_borrower].coll = Troves[_borrower].coll.add(pendingFURFIReward);
            Troves[_borrower].debt = Troves[_borrower].debt.add(pendingFURUSDDebtReward);

            _updateTroveRewardSnapshots(_borrower);

            // Transfer from DefaultPool to ActivePool
            _movePendingTroveRewardsToActivePool(_activePool, _defaultPool, pendingFURUSDDebtReward, pendingFURFIReward);
    ...
    function _updateTroveRewardSnapshots(address _borrower) internal {
        ...
        emit TroveSnapshotsUpdated(L_FURFI, L_FURUSDDebt);
    }
    ...
    function _redistributeDebtAndColl(IActivePool _activePool, IDefaultPool _defaultPool, uint _debt, uint _coll) internal {
        ...
        * error correction, to keep the cumulative error low in the running totals L_FURFI and L_FURUSDDebt:
        *
        ...
        */
        uint FURFINumerator = _coll.mul(DECIMAL_PRECISION).add(lastFURFIError_Redistribution);
        uint FURUSDDebtNumerator = _debt.mul(DECIMAL_PRECISION).add(lastFURUSDDebtError_Redistribution);
        ...
        lastFURFIError_Redistribution = FURFINumerator.sub(FURFIRewardPerUnitStaked.mul(totalStakes));
        lastFURUSDDebtError_Redistribution = FURUSDDebtNumerator.sub(FURUSDDebtRewardPerUnitStaked.mul(totalStakes));
    ...
    *
    * the total FURFI gas compensation from the liquidation sequence
    *
    * The FURFI as compensation must be excluded as it is always sent out at the very end of the liquidation sequence.
    */
    ...
    function _addTroveOwnerToArray(address _borrower) internal returns (uint128 index) {
        /* Max array size is 2**128 - 1, i.e. ~3e30 troves. No risk of overflow, since troves have minimum FURUSD
    ...
    */
    ...
    function _calcRedemptionFee(uint _redemptionRate, uint _FURFIDrawn) internal pure returns (uint) {
        ...
        require(redemptionFee < _FURFIDrawn, "TroveManager: Fee would eat up all returned collateral");
    ...
    function _requireFURUSDBalanceCoversRedemption(IFURUSDToken _furUSDToken, address _redeemer, uint _amount) internal view {
        require(_furUSDToken.balanceOf(_redeemer) >= _amount, "TroveManager: Requested redemption amount must be <= user's FURUSD token balance");
    }
